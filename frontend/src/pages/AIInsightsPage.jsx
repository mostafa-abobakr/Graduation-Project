    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Automated recommendations from your data
          </p>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-5 bg-card border-border/60 premium-shadow">
          ...
          <div className="stat-number text-foreground">{aiInsights.length}</div>
        </Card>
        <Card className="p-5 bg-card border-border/60 premium-shadow">
          <div className="text-sm text-muted-foreground mb-2">
            Potential Weekly Savings
          </div>
          <div className="stat-number text-primary">$1,120</div>
        </Card>
        <Card className="p-5 bg-card border-border/60 premium-shadow">
          <div className="text-sm text-muted-foreground mb-2">
            Avg. Confidence
          ...
          <div className="stat-number text-foreground">
            {Math.round(
              aiInsights.reduce((s, i) => s + i.confidence, 0) /
                aiInsights.length,
            )}
            %
          </div>
      ...
      <div className="space-y-3">
        {aiInsights.map((insight) => {
          ...
          return (
            <Card
              key={insight.id}
              className="p-5 bg-card border-border/60 hover:border-primary/20 transition-all premium-shadow"
            >
