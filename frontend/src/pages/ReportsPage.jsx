...
const BACKEND_BASE = "https://resturantai.runasp.net";
...
const safeFetchJson = async (url, options = {}) => {
  try {
    ...
    return { ok: false, status: null, data: null, error: error.message || "Network error" };
  }
};
...
export default function ReportsPage() {
  ...
  const exportDailyReportPdf = async (reportName = "Daily Report") => {
    if (!user?.restId) {
    ...
    try {
      ...
      const [
        ...
        safeFetchJson(`${ANALYTICS_BASE}/analytics/dashboard/${user.restId}`, {
          headers: { accept: "application/json" },
        }),
        safeFetchJson(`${ANALYTICS_BASE}/analytics/dashboard/revenue/${user.restId}`, {
          headers: { accept: "application/json" },
        }),
        safeFetchJson(`${ANALYTICS_BASE}/analytics/menu/performance/${user.restId}`, {
          headers: { accept: "application/json" },
        }),
        safeFetchJson(`${ANALYTICS_BASE}/inventory/items/${user.restId}`, {
          headers: { accept: "application/json" },
        }),
        safeFetchJson(`${ANALYTICS_BASE}/analytics/alerts/forecast/${user.restId}`, {
          method: "POST",
          headers: { accept: "application/json", "Content-Type": "application/json" },
          body: JSON.stringify({}),
        }),
        safeFetchJson(`${BACKEND_BASE}/api/Employees`, {
          headers: {
            "Content-Type": "application/json",
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
          },
        }),
        safeFetchJson(`${BACKEND_BASE}/api/Schedule/range?start_date=${start}&end_date=${end}`, {
          headers: {
            "Content-Type": "application/json",
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
          },
      ...
      const endpointRows = [
        ["GET", `/analytics/dashboard/${user.restId}`, dashboardRes.ok ? "OK" : `Failed (${dashboardRes.error})`],
        ["GET", `/analytics/dashboard/revenue/${user.restId}`, revenueRes.ok ? "OK" : `Failed (${revenueRes.error})`],
        ["GET", `/analytics/menu/performance/${user.restId}`, menuRes.ok ? "OK" : `Failed (${menuRes.error})`],
        ["GET", `/inventory/items/${user.restId}`, inventoryRes.ok ? "OK" : `Failed (${inventoryRes.error})`],
        ["POST", `/analytics/alerts/forecast/${user.restId}`, forecastAlertsRes.ok ? "OK" : `Failed (${forecastAlertsRes.error})`],
        ["GET", "/api/Employees", employeesRes.ok ? "OK" : `Failed (${employeesRes.error})`],
        ["GET", `/api/Schedule/range?start_date=${start}&end_date=${end}`, scheduleRes.ok ? "OK" : `Failed (${scheduleRes.error})`],
      ];
  ...
  return (
    <div className="space-y-5 animate-fade-in py-5">
      ...
      <PageHeader
        ...
        actions={
          <Button disabled={isLoading || isExporting} onClick={() => exportDailyReportPdf("Daily Report")}>
              ...
              return (
                <div key={report.id} className="flex items-center justify-between px-5 py-4 hover:bg-muted/20 transition-colors">
                  ...
                  <Button
                    ...
                    onClick={() => exportDailyReportPdf(report.name)}
                  >
